  ...
  return (
    <section id="section-stage-transfer" className="space-y-4">
      ...
      {stages.map(s => {
        ...
        const url       = `${scheme}://${stageHost}/api/v1/stage/${s.pid}`
        ...
        return (
          <div
            ...
            style={{
              border: `1px solid ${isSelected ? 'var(--blue-500)' : 'var(--border)'}`,
              backgroundColor: isSelected ? 'var(--blue-alpha)' : 'var(--surface)',
            }}
          >
            {/* Endpoint bar */}
            <div className="flex items-center gap-2 rounded-lg px-3 py-2" style={{ backgroundColor: 'var(--surface-2)' }}>
              ...
              {isSelected && (
                <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ color: 'var(--blue-500)', border: '1px solid var(--blue-500)' }}>active</span>
              )}
